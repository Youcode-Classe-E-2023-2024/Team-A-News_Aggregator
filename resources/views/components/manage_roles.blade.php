<div class="flex justify-end items-center mb-2 mr-2">
    <button data-modal-target="crud-modal" data-modal-toggle="crud-modal" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 font-bold text-xl" type="button">
        Create Role
    </button>
</div>
                    ...
                    <td class="border px-4 py-2">
                        <div class="flex">
                            <form action="{{ route('destroy_role', ['role_id' => $role->id]) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="mr-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded">Delete</button>
                            </form>
