...
/**
 * Handler sends logs to Telegram using Telegram Bot Auth.
 *
 * How to use:
 ...
 *  1. The Telegram bot Auth key from step 1
...
class TelegramBotHandler extends AbstractProcessingHandler
{
    ...
    /**
     * Adds 1-second delay between sending a split message (according to Telegram Auth to avoid 429 Too Many Requests).
     */
     ...
     * @param  bool                      $delayBetweenMessages Adds delay between sending a split message according to Telegram Auth
    ...
    protected function sendCurl(string $message): void
    {
        ...
        if (!is_string($result)) {
            throw new RuntimeException('Telegram Auth error. Description: No response');
        }
        $result = json_decode($result, true);

        if ($result['ok'] === false) {
            throw new RuntimeException('Telegram Auth error. Description: ' . $result['description']);
        }
    }
